...
// Calls the shared Vercel proxy which holds the Anthropic key server-side.
...
(function () {
  ...
  function buildWatchContext(watchedSeries) {
    ...
    ctx += "\n\n";
    ctx += "Use these titles to understand what genres, themes, and styles the user enjoys.\n";
  ...
  window.chikaRecommend = async function ({ filters, watchedSeries, similarTo, character, extra }) {
...
Prioritise anime available on Crunchyroll. Crunchyroll has an extensive catalog of simulcasts and licensed titles — prefer titles that are very likely to be on the platform.

Each object must have these fields:
{
  "title": "Exact anime title as known on Crunchyroll",
  "cr_url": "https://www.crunchyroll.com/search?q=TITLE_URL_ENCODED (use title-based search URL as fallback)",
  "genre": "Primary genre",
...
Prioritise diversity — don't repeat the same studio or franchise.`;
  ...
  window.chikaAsk = async function ({ userText, filters, watchedSeries }) {
    const system = `You are ChikaChan, a cheerful, knowledgeable anime recommendation AI for Crunchyroll.
Personality: warm, enthusiastic, concise. Use light anime vocabulary naturally.
Answer the user's question conversationally. If recommending, suggest 1–3 titles available on Crunchyroll with a brief reason each.`;

    const userContent = `${buildWatchContext(watchedSeries)}\n\n${buildFilterContext(filters)}\n\nUser: "${userText}"`;
